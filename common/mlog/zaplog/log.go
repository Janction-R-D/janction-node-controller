...
func New(opts ...conf.Option) *Log {
	...
	logger := newZapLogger(parseLevel(o.LogLevel), parseLevel(o.Stacktrace), o.CallerSkip, zapcore.NewMultiWriteSyncer(writers...))
	zap.RedirectStdLog(logger)
	logger = logger.With(zap.String("project", o.ProjectName)) //加上项目名称
	return &Log{logger: logger}
}

func newZapLogger(level, stacktrace zapcore.Level, callerSkip int, output zapcore.WriteSyncer) *zap.Logger {
	...
	return zap.New(zapcore.NewCore(encoder, output, dyn), zap.AddCaller(), zap.AddStacktrace(stacktrace), zap.AddCallerSkip(callerSkip))
}
