...
func newLogger(config *Config) *Logger {
	...
	if config.Debug {
		...
		ws = zapcore.AddSync(fileout.NewRollingFile(config.Path, config.ProjectName, config.MaxSize, config.MaxAge, config.MaxBackup))
	}
...
func DebugEncodeLevel(lv zapcore.Level, enc zapcore.PrimitiveArrayEncoder) {
	var colorize = mcolor.Red
	switch lv {
	case zapcore.DebugLevel:
		colorize = mcolor.Blue
	case zapcore.InfoLevel:
		colorize = mcolor.Green
	case zapcore.WarnLevel:
		colorize = mcolor.Yellow
...
func panicDetail(msg string, fields ...Field) {
	...
	// 控制台输出
	fmt.Printf("%s: \n    %s: %s\n", mcolor.Red("panic"), mcolor.Red("msg"), msg)
	for key, val := range enc.Fields {
		fmt.Printf("    %s: %s\n", mcolor.Red(key), fmt.Sprintf("%+v", val))
	}
}
